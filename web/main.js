let pages=['homePage','tablePage','dishPage','orderPage','createTablePage','editTablePage',
            'dishCreatePage','editDishPage','createOrderPage'];
...
create_order_btn.addEventListener('click',function(event){
    ...
    let dish_count=document.querySelector("#create_dish_count").value;
...
function loadOrderByTable(id){
    ...
    let orderStr="";
    eel.getOrderByTable(id)(function(data){
        currentOrders=data;
        let grandTotal=0;
        data.forEach((order)=>{
...
function loadDish(){
    eel.getDishes()(function(data){
        dishStr='';
        data.forEach((dish)=>{
            let change=dish.status== 1 ? 'success' : 'secondary';
            dishStr+=`
            <tr>
                <td>${dish.id}</td>
                <td>${dish.name}</td>
                <td>${dish.price}</td>
                <td>
                    <button class="btn btn-${change} btn-sm" onclick="updateDishStatus(${dish.id},${dish.status})">${dish.status}</button>
                </td>
                <td>
                    <button class="btn btn-warning btn-sm" onclick="getDishById(${dish.id})"><i class="fa fa-edit"></i></button>
                    <button class="btn btn-danger btn-sm" onclick="deleteDish(${dish.id})"><i class="fa fa-trash"></i></button>
                </td>
            </tr>
         `;
        });
...
function loadTable(){
    eel.getTables()(function(data){
       tableStr='';
       data.forEach((table)=>{
           let change=table.status== 1 ? 'success' : 'secondary';
           tableStr+=`
            <tr>
                <td>${table.id}</td>
                <td>${table.name}</td>
                <td>
                    <button class="btn btn-${change} btn-sm" onclick="updateTableStatus(${table.id}
                        ,${table.status})">${table.status}</button>
                    <button class="btn btn-${change} btn-sm" onclick="loadOrderByTable(${table.id})">See Order</button>
                </td>
                <td>
...
create_table_btn.addEventListener('click',function(event){
    ...
    eel.createTabe(table_name)(function(){
        loadTable();
        managePage(pages[1]);
...
create_dish_btn.addEventListener('click',function(event){
    ...
    eel.createDish(dish_name,dish_price)(function(){
        loadDish();
        managePage(pages[2]);
...
function getDishById(id){
    eel.getDishById(id)(function(dish){
        document.querySelector("#edit_dish_name").value=dish.name;
        document.querySelector("#edit_dish_price").value=dish.price;
        document.querySelector("#edit_dish_id").value=dish.id;
        managePage(pages[7]);
    })
}
...
edit_dish_btn.addEventListener('click', function(event){
    event.preventDefault();
    let new_dish_name=document.querySelector("#edit_dish_name").value;
    let edit_dish_price=document.querySelector("#edit_dish_price").value;
    let dish_id=document.querySelector("#edit_dish_id").value;
    eel.updateDish(dish_id,new_dish_name,edit_dish_price)(function(data){
        loadDish();
        managePage(pages[2]);
    })
})
...
function updateDishStatus(id,status){
    status=status== 1 ? 0 : 1;
    eel.updateDishStatus(id,status)(function(data){
        loadDish();
    })
}